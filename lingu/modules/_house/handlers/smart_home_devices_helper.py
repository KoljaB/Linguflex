...
class SmartBulbThread(threading.Thread):
    ...
    def set_bulb_on_off(self, 
        ...
        with self.action_condition:
            ...
            log.low(f'  [house] thread of {self.bulb_param["name"]} set to {state}')
    ...
    def change_color(self, 
        ...
        with self.action_condition:
            ...
            #log.low(f'  [house] notifying color_change in thread for bulb {self.bulb_param["name"]}')
            self.action_condition.notify()        
    ...
    def run(self) -> None:
        ...
        log.low(f'  [house] 💡 bulb {name} connected')        
        ...
        log.low(f'  [house] {name} color: {self.color}, device state: {self.state}')
        ...
        while self.is_running:
            with self.action_condition:
                ...
                #     #log.low(f'  [house] power_change_condition notification')
                #     if self.state == 'on':
                #         self.device.turn_on()
                #         log.low(f'  [house] {self.bulb_param["name"]} turned on')
                #     elif self.state == 'off':
...
class OutletDeviceThread(threading.Thread):
    ...
    def set_state(self, 
            state: bool) -> None:
        self.state = state
        log.low(f'  [house] {self.outlet_param["name"]} state set to: {state}')        
        if state:
    ...
    def check_state(self):
        ...
        if 'dps' in data:
            dps = data['dps']
            #print (f"  [house] 🔌 {self.outlet_param['name']} DPS: {dps}")
            self.state = dps['1']
            self.watt = dps['19']
            #log.low(f'  [house] 🔌 {self.outlet_param["name"]} STATE: {self.state} WATT: {self.watt}')
        #log.inf(f'  [house] 🔌 {name} STATE: {self.state}')
    ...
    def run(self) -> None:
        ...
        # print(f"  [house] 🔌 connecting to name {name}, id {id}, ip {ip}, key {key}, deviceVersion {deviceVersion}")
        self.device = tinytuya.OutletDevice(id, ip, key)
        self.device.set_version(deviceVersion)
        log.low(f'  [house] 🔌 device {name} connected')        
        self.connection_event.set()
...
class OutletManager():
    ...
    def get_states(self, 
        ...
        for outlet_name in outlet_names:
            found = False
            for outlet_thread in self.outlet_threads:
                # print (f"Outlet {outlet_thread.outlet_param['name']} {outlet_thread.state}")
                if outlet_thread.outlet_param['name'].lower() == outlet_name.lower():
                    i = self.find_outlet_index(outlet_name)
                    if i != -1:
                        results.append({

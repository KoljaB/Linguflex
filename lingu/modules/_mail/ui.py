...
class MailUI(UI):
    ...
    def update_current_mail(self):
        ...
        #     print(f"📫  MAILS before sort: {mail['raw_date']}")

        # Sort mails by their date in descending order
        # self.mails.sort(
        #     key=lambda mail:
        #     parse(mail['raw_date']),
        #         reverse=True)
        ...
        # self.mails.sort(key=lambda mail: parse(mail['raw_date']), reverse=True)

        # for mail in self.mails:
        #     print(f"📫  MAILS after sort: {mail['raw_date']}")

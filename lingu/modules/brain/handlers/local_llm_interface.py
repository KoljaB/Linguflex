...
class LocalLLMInterface(LLM_Base):
    def __init__(
        ...
        log.dbg("Warming up HermesInterface")
        self.warm_up()
        log.dbg("HermesInterface warmed up")
    ...
    def decide_for_tool_to_call(self, messages, tools):
        ...
        print(f"  [brain] {self.model_name} wants to call function {fct_call}")
        ...
        print(f"  [brain] {self.model_name} tool found: {tool}")
        print(f"  [brain] tool type of instance: {type_of_tool}")

        messages_tool = [
            user_message
        ]

        print(f"  [brain] calling tool {fct_call} with messages {messages}")
        try:
            extraction_stream = self.create(
            ...
            print(f"  [brain] tool {fct_call} failed, reason: {str(e)}")
            exc(e)
        ...
        print(f"  [brain] {fct_call} executed, return value: {return_value}")
    ...
    def generate(self, messages, tools=None):
        log.dbg(f"  [brain] {self.model_name}"
                " generating answer for messsages:"
                f" {messages}")

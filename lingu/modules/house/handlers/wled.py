...
class WLED_Handler():   
    def __init__(self, wled_url, max_bulbs) -> None: 

        self.wled_url = wled_url
        ...
        file_path = "lingu/config/wled.json"
    ...
    def bulb_colors_changed(self, bulb_colors): 
    ...
    def addcolor(self,
            ...
            index: int) -> None: 
        log.dbg(f'  [wled] add color {rgb} at {index}')
    ...
    def set_wled_colors(self, 
        ...
        self.simple_json_post(self.wled_url, complete_json)

    def simple_json_post(self, 
            url: str, json: str) -> None:
        headers = {'Content-Type': 'application/json'}
        try:
            log.dbg(f'  [wled] setting colors') # {json}
            response = requests.post(url, data=json, headers=headers)
            if response.status_code == 200:
                result = response.text
                log.dbg(f'  [wled] response: {result}') 
        except Exception as ex:
            log.dbg(f'[wled] ERROR sending json {json}, error: {str(ex)}') 

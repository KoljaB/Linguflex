...
disabled_response = {
    "result": "error",
    "reason": "House module is disabled. Please configure devices in the config file."
}
...
class get_bulb_light_colors(Populatable):
    """
...
class set_smart_device_on_off(Populatable):
    ...
    def on_populated(self):
        ...
        log.dbg(f"  [device] set_smart_device_on_off: {self.names} "
                f"to {self.turn_on}")
        ...
        for name in self.names:
            ...
            if name in logic.get_bulb_names():
                ...
                log.dbg(f"  [house] {name} not found")
                log.dbg(f"  [house] bulbs: {logic.get_bulb_names()}")
                log.dbg(f"  [house] outlets: {logic.get_outlet_names()}")
                result[name] = {
                    "result": "error",

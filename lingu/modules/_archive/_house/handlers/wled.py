...
class WLED_Handler():   
    ...
    def addcolor(self,
            ...
            index: int) -> None: 
        log(Level.Dbg, f'  [wled] add color {rgb} at {index}')
    ...
    def simple_json_post(self, 
        ...
        try:
            log(Level.Dbg, f'  [wled] setting colors') # {json}
            response = requests.post(url, data=json, headers=headers)
            if response.status_code == 200:
                result = response.text
                log(Level.Dbg, f'  [wled] response: {result}') 
        except Exception as ex:
            log(Level.Dbg, f'[wled] ERROR sending json {json}, error: {str(ex)}') 
